  ...
  describe("error filtering", () => {
    ...
    it("scrubs sensitive query parameters from URLs", async () => {
      ...
      window.location = new URL(
        "https://example.com/page?token=secret&user=jules",
      ) as any;
      Object.defineProperty(document, "referrer", {
        value: "https://referrer.com/?api_key=123",
        configurable: true,
      });
      ...
      const event = await parseEvent(0);
      expect(event.contexts.page.url).toBe(
        "https://example.com/page?token=[filtered]&user=jules",
      );
      expect(event.contexts.page["http.query"]).toBe(
        "?token=[filtered]&user=jules",
      );
      expect(event.contexts.page.referer).toBe(
        "https://referrer.com/?api_key=[filtered]",
      );
    ...
    it("scrubs Basic Auth credentials from URLs", async () => {
      const origLocation = window.location;
      delete (window as any).location;
      window.location = new URL("https://user:password@example.com/api") as any;

      const client = new CentryClient({ project: "p" });
      client.captureException(new Error("test auth"));
      await drain();

      const event = await parseEvent(0);
      expect(event.contexts.page.url).toBe(
        "https://[filtered]:[filtered]@example.com/api",
      );

      window.location = origLocation;
    });

    it("scrubs query-style URL fragments and preserves plain hash/SPA routes", async () => {
      const origLocation = window.location;
      delete (window as any).location;
      window.location = new URL(
        "https://example.com/#access_token=secret&tab=security",
      ) as any;

      const client = new CentryClient({ project: "p" });
      client.captureException(new Error("test hash"));
      await drain();

      const event = await parseEvent(0);
      expect(event.contexts.page.url).toBe(
        "https://example.com/#access_token=[filtered]&tab=security",
      );

      // Test plain SPA route preservation
      setupMocks();
      window.location = new URL(
        "https://example.com/#/dashboard/overview",
      ) as any;
      client.captureException(new Error("test plain spa"));
      await drain();

      const event2 = await parseEvent(1);
      expect(event2.contexts.page.url).toBe(
        "https://example.com/#/dashboard/overview",
      );
...
describe("module-level singleton", () => {
  ...
  it("supports opting out of automatic browser global handlers", async () => {
    ...
    window.dispatchEvent(
      new ErrorEvent("error", {
        error: new Error("ignored crash"),
        message: "ignored crash",
      }),
    );
    await drain();
  ...
  it("reuses one listener installation across repeated init calls", async () => {
    ...
    window.dispatchEvent(
      new ErrorEvent("error", {
        error: new Error("reinit crash"),
        message: "reinit crash",
      }),
    );
    await drain();
  ...
  it("cleans up manual handlers correctly and supports re-install", async () => {
    const cleanup = installGlobalHandlers(
      new CentryClient({ project: "manual-app" }),
    );

    window.dispatchEvent(
      new ErrorEvent("error", {
        error: new Error("manual crash"),
