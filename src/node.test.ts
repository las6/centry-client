...
import { NodeClient, initNode, captureException, captureMessage, getNodeClient, withCentry, installNodeGlobalHandlers } from './node'
...
describe('NodeClient', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('{"id":"issue1"}', { status: 200 })))
  ...
  describe('event capture', () => {
    ...
    it('includes event metadata', async () => {
      ...
      expect((event.exception as Record<string, unknown[]>).values[0]).toMatchObject({
        type: 'Error',
    ...
    it('includes environment and release', async () => {
      const configured = new NodeClient({ project: 'p', environment: 'prod', release: '1.0.0' })
    ...
    it('sets runtime context to node with version', async () => {
      ...
      const event = parsedEvent(fetchCalls()[0].body)
      const runtime = (event.contexts as Record<string, unknown>).runtime as Record<string, string>
...
describe('installNodeGlobalHandlers', () => {
  ...
  it('captures uncaughtException events', async () => {
    ...
    process.emit('uncaughtException', new Error('global crash') as Error & { domainEmitter?: unknown })
    await vi.runAllTimersAsync()
  ...
  it('captures unhandledRejection events', async () => {
    ...
    process.emit('unhandledRejection', new Error('promise rejected'), Promise.resolve())
    await vi.runAllTimersAsync()
  ...
  it('cleanup removes the handlers', () => {
    ...
    process.emit('uncaughtException', new Error('after cleanup') as Error & { domainEmitter?: unknown })
    vi.runAllTimers()
    expect(fetchCalls()).toHaveLength(0)
  })
})
...
describe('withCentry (Node)', () => {
  ...
  it('captures unhandled errors from the handler', async () => {
    ...
    const calls = fetchCalls().filter(c => c.url.includes('/api/p/envelope/'))
    expect(calls.length).toBeGreaterThan(0)
    const event = parsedEvent(calls[0].body)
    expect((event.exception as Record<string, unknown[]>).values[0]).toMatchObject({
  ...
  it('attaches Web Request context via ALS', async () => {
    ...
    const errorCall = calls.find(c => {
      ...
      return (event.exception as Record<string, unknown[]> | undefined) !== undefined
    })
    expect(errorCall).toBeDefined()
    const event = parsedEvent(errorCall!.body)
    expect((event.request as Record<string, unknown>)?.url).toBe('https://api.example.com/data')
  })
