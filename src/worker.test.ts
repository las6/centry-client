...
import { WorkerClient, initWorker, captureWorkerException, getWorkerClient, withCentry } from './worker'
...
describe('WorkerClient', () => {
  ...
  describe('event capture', () => {
    ...
    it('includes environment and release', async () => {
      const configured = new WorkerClient({ project: 'p', environment: 'prod', release: '2.0.0' })
  ...
  describe('request context', () => {
    ...
    it('keeps referer but scrubs sensitive query parameters from it', async () => {
      const req = {
        ...
        url: '/v1/user',
        headers: {
          'referer': 'https://dashboard.example.com/reset?token=secret&tab=security',
        },
      }
      client.captureException(new Error('test'), req)
      await vi.runAllTimersAsync()
  ...
  describe('toError conversion', () => {
    ...
    it('handles object with message property', async () => {
      client.captureException({ message: 'obj error', name: 'CustomError' } as unknown as Error)
      await vi.runAllTimersAsync()
